  ...
  return <section className="plugins-page">
    ...
    <div className="plugins-page-content">
      ...
      {tab === "discover" ? <div className="marketplace-browser">
        ...
        {looseResources.length > 0 && <div className="loose-resources"><h3>本地资源</h3>{looseResources.map((item) => <button key={`${item.kind}-${item.path}`} onClick={() => void openPath(item.path).catch(() => undefined)}><span className={`resource-kind ${item.kind}`}>{({ extension: "扩展", skill: "技能", prompt: "提示词", theme: "主题", pet: "宠物" } as Record<string, string>)[item.kind] || item.kind}</span><span><strong>{item.name}</strong><small>{item.path}</small></span><em>{item.scope === "project" ? "项目" : "用户"}</em></button>)}</div>}
      </section>}
